...
void stage2(double *A, int offset, int *sizedim, int *start, int N, int M)
{
    ...
    for (i=1;i<M;i++){
        ...
        printf("block %d,%d\n", offset, i);
        printf("col = %d, B2 = %d, row = %d",  (x+start[i])*N+y, x*N+y, x*N+(y+start[i]));


        L2 = sizedim[i];
        L3 = sizedim[i];
#pragma omp task firstprivate(i, L1, L2,x, y, N)
        ProcessBlockOnColumn( &A[(x+start[i])*N+y], 
                              &A[ x          *N+y],
                              L1, L2, N);
#pragma omp task firstprivate(i, L1, L2,x, y, N)
        ProcessBlockOnRow( &A[x*N+(y+start[i])],
                           &A[x*N+ y          ],
                           L1, L3, N);
    }
#pragma omp taskwait
...
void stage3(double *A, int offset, int *sizedim, int *start, int N, int M)
{
    ...
    for (i=1;i<M;i++)
        for (j=1;j<M;j++){
            ...
            printf("block %d,%d\n",i, offset);
            printf("\tInner, B1 = %d, B2 = %d, B3 = %d\n", (x+start[i])*N + (y+start[j]), x*N + (y+start[j]), (x+start[i])*N + y );
